    ...
    <div className="w-full flex flex-col items-center">
      ...
      >
        ...
        {Array.from({ length: playerCount }, (_, i) => i + 1).map(num => (
          <ClockSlice key={num} num={num} playerCount={playerCount} isCurrent={num === playerNo} isVoter={isVoting && pendingNom?.voters.includes(num.toString())} isDead={deadPlayers.includes(num)} innerRadius={innerRadius} outerRadius={outerRadius} ringCount={ringCount} ringWidth={ringWidth} votedAtDay={votedAtDay[num.toString()] || {}} deathInfo={deaths.find(d => d.playerNo === num.toString())} showDeathIcons={showDeathIcons} mode={mode} assignmentMode={assignmentMode || null} onStart={(e) => handleStart(num, e)} showAxis={showAxis} />
        ))}
        {!isVoting && mode === 'allReceive' && arrowData.map((a, idx) => <ClockArrow key={idx} {...a} playerCount={playerCount} innerRadius={innerRadius} ringWidth={ringWidth} width={mode === 'allReceive' ? 1.5 : 2.5} />)}
        {pendingNom && <ClockArrow from={parseInt(pendingNom.f)} to={parseInt(pendingNom.t)} day={ringCount} type={pendingNom.f === pendingNom.t ? 'self' : 'to'} playerCount={playerCount} innerRadius={innerRadius} ringWidth={ringWidth} width={3} />}
