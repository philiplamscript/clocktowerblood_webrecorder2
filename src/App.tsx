" which is incomplete. It looks like there's a malformed class name.
[REDACTED]</think><dyad-write path="src/components/PlayerDetailView.tsx" description="Fixing the incomplete class name in PlayerDetailView.tsx that was corrupted during the previous edit.">
"use client";
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            {isDead ? (
              ...
              <button 
                onClick={() => togglePlayerAlive(playerNo)}
                className="flex-[8] h-10 rounded-lg text-[10px] font-black uppercase transition-all flex items-center justify-center gap-2 shadow-sm bg-emerald-600"
              >
                <span>ALIVE</span>
              </button>
            )}
            <input 
              ...
              className="flex-[4] h-10 bg-white border rounded-lg px-3 text-[10px] font-bold focus:ring-0" 
            />
          </div>
        </div>
      </div>
